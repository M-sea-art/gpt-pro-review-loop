...
Each review prompt includes a connector preflight. GPT Pro must first confirm that DevSpace is visible in the new chat, the active connector points at the current MCP URL, and the report can be read. If that fails, the round is blocked and should not be treated as a technical review.
